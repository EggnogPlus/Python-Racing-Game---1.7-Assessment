...
# track_number = 0
def track_check(low_boundary, high_boundary, track_number_value):
    valid = False

    while not valid:

        try:
            track_number_value = int(input('now select a track length between: 5 & 15?'))
            if low_boundary <= track_number_value <= high_boundary:
                print("you have selected track number - {}. Good luck racing!".format(track_number_value))
                # break
                return track_number_value
            else:
...
print(
    'Here is how far every car got in the race (Length:{})- Car 1: {}, Car 2: {}, Car 3: {}, Car 4: {}, Car 5: {}, Car 6: {}'.format(
